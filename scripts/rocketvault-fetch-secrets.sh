...
# Optional env vars: VAULT_ENV_FILE, VAULT_INSECURE, VAULT_NAME (target vault; unset/empty
#   uses the legacy default-vault path)
...
# Build the secrets base path — vault-scoped when VAULT_NAME is set, else legacy default-vault path.
if [ -n "${VAULT_NAME:-}" ]; then
  _rv_secrets_path="/api/v1/vaults/${VAULT_NAME}/secrets"
else
  _rv_secrets_path="/api/v1/secrets"
fi
...
for _rv_pair in ${VAULT_SECRETS}; do
  ...
  _rv_http_status=$(
    curl -s ${_rv_curl_flags} \
      ...
      "${VAULT_URL}${_rv_secrets_path}/${_rv_uuid}"
  )
  ...
  if [ "${_rv_http_status}" != "200" ]; then
    ...
    unset _rv_token _rv_curl_flags _rv_pair _rv_var _rv_uuid _rv_http_status _rv_secret_body _rv_secrets_path
    return 1 2>/dev/null || exit 1
  fi
  ...
  if [ -z "${_rv_value}" ]; then
    ...
    rm -f "${_rv_secret_body}"
    unset _rv_token _rv_curl_flags _rv_pair _rv_var _rv_uuid _rv_http_status _rv_value _rv_secret_body _rv_secrets_path
    return 1 2>/dev/null || exit 1
  fi
...
unset _rv_secret_body _rv_escaped _rv_secrets_path
unset -f _rv_error
