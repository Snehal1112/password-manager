...
func (r *TagRepository[T]) AddTags(ctx context.Context, id int, tags []string) error {
	...
	for _, tag := range tags {
		...
		if err != nil {
			if sqliteErr, ok := err.(sqlite3.Error); ok && sqliteErr.Code == sqlite3.ErrConstraint {
				logrus.Warnf("Skipping duplicate tag %s for %s %d", tag, r.idColumn, id)
				continue
			}
			logrus.Error("Failed to insert tag: ", err)
			return fmt.Errorf("failed to insert tag %s: %w", tag, err)
		}
		logrus.Debugf("Inserted tag %s for %s %d", tag, r.idColumn, id)
	}
...
func (r *TagRepository[T]) GetTags(ctx context.Context, id int) ([]string, error) {
