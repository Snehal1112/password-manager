...
func TestGenerateRSA(t *testing.T) {
	...
	key, err := repo.GenerateRSA(ctx, 1, "test-rsa-key", 2048, []string{"prod"})
	assert.NoError(t, err, "generating RSA key should succeed")
	err = repo.Create(ctx, *key)
	if err != nil {
		rows, err2 := sqlDB.Query("SELECT name FROM sqlite_master WHERE type='table'")
		if err2 == nil {
			t.Log("Database tables:")
			for rows.Next() {
				var name string
				rows.Scan(&name)
				t.Logf("- %s", name)
	...
	assert.NoError(t, err, "creating key with tags should succeed")

	var id int
	var name, value, keyType string
	var revoked bool
	err = sqlDB.QueryRow("SELECT id, name, value, type, revoked FROM keys WHERE user_id = ?", 1).
	...
	t.Logf("Querying tag for key_id %d: %v", id, err)
...
func TestGenerateECDSA(t *testing.T) {
	...
	ctx := context.Background()
	repo := NewKeyRepository(sqlDB, log)
	key, err := repo.GenerateECDSA(ctx, 1, "test-ecdsa-key", "P-256", []string{"dev"})
...
func TestListByUser(t *testing.T) {
	...
	_, err = db.DB.Exec(
		"INSERT INTO keys (id, user_id, name, value, type, revoked, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
		1, 1, "test-key1", encryptedValue1, "RSA", false, createdAt,
	)
	assert.NoError(t, err, "inserting key 1 should succeed")
	_, err = db.DB.Exec(
		"INSERT INTO keys (id, user_id, name, value, type, revoked, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
		2, 1, "test-key2", encryptedValue2, "ECDSA", false, createdAt,
	)
	...
	keysList, err := repo.ListByUser(ctx, 1, "RSA", []string{"prod"})

	assert.NoError(t, err, "listing keys should succeed")
	assert.Len(t, keysList, 1, "should return one key")
	assert.Equal(t, 1, keysList[0].ID, "key ID should match")
...
func TestRotate(t *testing.T) {
	...
	_, err = db.DB.Exec(
		"INSERT INTO keys (id, user_id, name, value, type, revoked, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
		1, 1, "test-key", encryptedValue, "RSA", false, time.Now(),
	)
	assert.NoError(t, err, "inserting key should succeed")
	...
	err = db.DB.QueryRow("SELECT id, user_id, name, value, type, revoked FROM keys WHERE id = ?", 1).Scan(&ID, &userID, &name, &value, &types, &revoked)
	...
	err = db.DB.QueryRow("SELECT id FROM keys WHERE user_id = ? AND name = ? AND id != ?", 1, "test-key", 0).Scan(&newID)
