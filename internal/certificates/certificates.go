...
type CertificateRepository interface {
	CreateSelfSigned(ctx context.Context, userID uuid.UUID, name string, keyID uuid.UUID, validityDays int, tags []string) (*Certificate, error)
	CreateCASigned(ctx context.Context, userID uuid.UUID, name string, keyID uuid.UUID, caCertID uuid.UUID, validityDays int, tags []string) (*Certificate, error)
	Read(ctx context.Context, id uuid.UUID) (*Certificate, error)
	...
	ListByUser(ctx context.Context, userID uuid.UUID, certType string, tags []string) ([]Certificate, error)
	ListRevoked(ctx context.Context, userID uuid.UUID) ([]RevokedCertificate, error)
}
...
func (r *certificateRepository) CreateSelfSigned(ctx context.Context, userID uuid.UUID, name string, keyID uuid.UUID, validityDays int, tags []string) (*Certificate, error) {
	keyRepo := keys.NewKeyRepository(r.db, r.log)
	key, err := keyRepo.Read(ctx, keyID)
	if err != nil {
		r.log.LogAuditError(userID.String(), "create_self_signed", "failed", "Failed to read private key", err)
		return nil, fmt.Errorf("failed to read private key: %w", err)
	}

	privateKey, err := parsePrivateKey(key.Value, key.Type)
	if err != nil {
		r.log.LogAuditError(userID.String(), "create_self_signed", "failed", "Failed to parse private key", err)
	...
	if err != nil {
		r.log.LogAuditError(userID.String(), "create_self_signed", "failed", "Failed to create certificate", err)
	...
	if err := r.storeCertificate(ctx, cert); err != nil {
		r.log.LogAuditError(userID.String(), "create_self_signed", "failed", "Failed to store certificate", err)
		return nil, err
	}

	r.log.LogAuditInfo(userID.String(), "create_self_signed", "success", "Self-signed certificate created successfully")
...
// - tags: The tags to associate with the certificate.
// Returns: An error if certificate creation or storage fails.
func (r *certificateRepository) CreateCASigned(ctx context.Context, userID uuid.UUID, name string, keyID uuid.UUID, caCertID uuid.UUID, validityDays int, tags []string) (*Certificate, error) {
	keyRepo := keys.NewKeyRepository(r.db, r.log)
	key, err := keyRepo.Read(ctx, keyID)
	if err != nil {
		r.log.LogAuditError(userID.String(), "create_ca_signed", "failed", "Failed to read private key", err)
		return nil, fmt.Errorf("failed to read private key: %w", err)
	}

	privateKey, err := parsePrivateKey(key.Value, key.Type)
	if err != nil {
		r.log.LogAuditError(userID.String(), "create_ca_signed", "failed", "Failed to parse private key", err)
		return nil, fmt.Errorf("failed to parse private key: %w", err)
	}

	caCert, err := r.Read(ctx, caCertID)
	if err != nil {
		r.log.LogAuditError(userID.String(), "create_ca_signed", "failed", "Failed to read CA certificate", err)
	...
	if err != nil {
		r.log.LogAuditError(userID.String(), "create_ca_signed", "failed", "Failed to parse CA certificate", err)
		return nil, fmt.Errorf("failed to parse CA certificate: %w", err)
	}

	caPrivateKey, err := parsePrivateKey(caCert.PrivateKey, "RSA") // Assume CA key is RSA; extend if needed
	if err != nil {
		r.log.LogAuditError(userID.String(), "create_ca_signed", "failed", "Failed to parse CA private key", err)
	...
	if err != nil {
		r.log.LogAuditError(userID.String(), "create_ca_signed", "failed", "Failed to create CA-signed certificate", err)
	...
	if err := r.storeCertificate(ctx, cert); err != nil {
		r.log.LogAuditError(userID.String(), "create_ca_signed", "failed", "Failed to store certificate", err)
		return nil, err
	}

	r.log.LogAuditInfo(userID.String(), "create_ca_signed", "success", "CA-signed certificate created successfully")
	logrus.WithFields(logrus.Fields{
		"user_id": userID.String(),
		"name":    name,
...
func (r *certificateRepository) Read(ctx context.Context, id uuid.UUID) (*Certificate, error) {
...
func (r *certificateRepository) Update(ctx context.Context, cert *Certificate) error {
	...
	_, err = tx.ExecContext(
		...
		cert.Name, cert.Certificate, encryptedPrivateKey, cert.CreatedAt, cert.ID.String(),
	)
	if err != nil {
...
func (r *certificateRepository) Delete(ctx context.Context, id uuid.UUID) error {
	...
	_, err = tx.ExecContext(ctx, "DELETE FROM certificates WHERE id = ?", id.String())
	if err != nil {
...
func (r *certificateRepository) ListByUser(ctx context.Context, userID uuid.UUID, certType string, tags []string) ([]Certificate, error) {
...
func (r *certificateRepository) ListRevoked(ctx context.Context, userID uuid.UUID) ([]RevokedCertificate, error) {
...
func (r *certificateRepository) storeCertificate(ctx context.Context, cert Certificate) error {
	...
	_, err = tx.ExecContext(
		...
		cert.ID.String(), cert.UserID.String(), cert.Name, cert.Certificate, encryptedPrivateKey, cert.CreatedAt,
	)
	if err != nil {
