...
- **A remote-target guard still blocks most commands, but `secrets` has broken out of it.** `persistentPreRun` resolves a remote target via `cliclient.ResolveTarget`: `--server` flag > `ROCKETVAULT_ADDR` env > the active `rocketvault context` (saved with `context add`, selected with `context use`). Every `secrets` subcommand — `create`, `get`, `list`, `update`, `delete`, `export`, `import` — now has its own remote-mode adapter (`remoteCapableSecretsCommands` in `cmd/root.go`, backed by `internal/cliclient`) and genuinely talks to `target.Server` over HTTP instead of the local service container when a remote target is resolved. Every other resource command (`keys`, `certificate`, `vaults`, `vault-access`, `users`, `audit`) has no adapter yet: with a remote target resolved, those are refused outright with `remote mode (...) is not yet supported for %q; unset it to run against the local instance` — the same message this doc used to report for `secrets` too. `context` itself, Cobra's own built-ins (`help`/`completion`), and the two commands that touch no server at all (`vault-access roles`, `secrets generate-password`) are exempt from the guard regardless. `--server`/`--ca-cert`/`--insecure-skip-verify` are registered as persistent flags on every command either way. See the comment above the guard in `cmd/root.go` for the plan to retire it group-by-group as each remaining command gains real remote support.
...
- **In local mode, the CLI talks to the service container directly** (`internal/container.ServiceContainerInterface`), bypassing the HTTP API and its middleware chain — so the API-level `rate_limit` config in `.rocketvault.yaml` does not apply. This no longer holds for `secrets` commands run against a remote target (see the remote-target gotcha above): those genuinely go over HTTP to `target.Server` and are subject to that server's rate limits like any other API client.
