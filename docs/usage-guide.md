...
- **System commands skip auth entirely.** `health`, `serve`, `admin`, `migrate` / `migrate:status` / `migrate:to` / `migrate:create`, `roles`, and `preview-migration` (i.e. `vaults preview-migration`) are listed in `systemCmds` in `cmd/root.go` and run without `--username` / `--password`. `backup` used to be in this list too (a real, unauthenticated-dump vulnerability) — it's since been removed and now requires an admin login like every other data-touching command; see the Backup / restore tooling section's gotchas.
