...
**What it is:** The `rocketvault` Cobra-based command-line binary. The same binary that runs the server (`rocketvault serve`) also exposes `secrets`, `keys`, `certificate` (singular), `users`, and `vaults` subcommands that talk directly to the service layer — no HTTP hop — authenticating each invocation with username, password, and TOTP code. Other registered top-level groups include `audit`, `vault-access`, `rotation` (under `secrets`), `migrate`, `backup`, and `health`.
...
- An existing user account (username + password + TOTP secret) for any non-system command. For the very first user, you need the `bootstrap_token` config key (see `.rocketvault.yaml`) and `rocketvault users admin`.
...
# Log in (prints a JWT; every other command re-authenticates itself, this is just to verify credentials)
...
- A service account (OAuth2 client) already created; the `service_account` role already carries `get` / `list` on `secrets`, so an access policy is not what grants the read (see section 3) — service accounts are read-only consumers; only admins create secrets (see `docs/consuming-secrets-guide.md` Steps 1–2).
...
- For `/api/v1/health/database`: a valid `Authorization: Bearer <jwt>` header from any authenticated user (no specific role or permission is required — see the gotchas below). For `/api/v1/health`, `/api/v1/health/ready`, and `/api/v1/health/live`: no token needed.
- Nothing extra is required to *record* crypto-latency samples — the histogram is wired into `CryptoService`'s operations automatically via the service container (`internal/services/keys/crypto_service.go`). There is currently no HTTP endpoint that exposes the histogram for scraping (see the gotchas).
...
- **Auth is asymmetric across the four routes.** `AuthenticationMiddleware` and `AuthorizationMiddleware` in `internal/middleware/middleware.go` explicitly skip `/health`, `/health/ready`, and `/health/live` — those three are always public. `/health/database` is *not* in that skip list, so it requires a valid `Authorization: Bearer` token like any other API route. However, `RBACService.mapEndpointToPermission` has no mapping for `/health/*`, so once authenticated, any role (`admin` or `user`) is allowed through `AuthorizationMiddleware` — there's no dedicated "health-check" permission to grant or revoke.
- **Health endpoints are still rate-limited.** They go through `RateLimitMiddleware` like every other route on `ApiRoot`, so the default limiter (`rate_limit.default: 300` requests/min per IP in `.rocketvault.yaml`) applies. Aggressive external probing — for example, multiple load balancers polling from the same NAT'd IP — can hit `429 Too Many Requests`; there is no per-endpoint exemption for health checks.
...
- **`monitoring.*` and `health.*` keys in `.rocketvault.yaml` are currently dead stubs.** The config file ships `monitoring.enable_metrics`, `monitoring.metrics_interval`, `monitoring.slow_query_threshold`, `health.check_interval`, `health.database_timeout`, and `health.enable_detailed_metrics`, but no non-test Go code reads any of them via `viper.Get*`. They don't configure anything today — the health handlers always compute full metrics on every request, with no server-side polling interval or timeout override.
- **No `/metrics` HTTP endpoint exists yet.** `internal/metrics/crypto_metrics.go` defines a Prometheus `HistogramVec` named `rocketvault_crypto_op_duration_seconds` (labels `op`, `key_type`, `cache_hit`; buckets from `0.001` to `0.5` seconds), and `NewDefaultPrometheusCryptoMetrics()` registers it on Prometheus's default registry via `internal/container/service_container.go`. `internal/services/keys/crypto_service.go` calls `RecordOp` after every `Sign` / `Verify` / `Encrypt` / `Decrypt` / `WrapKey` / `UnwrapKey` operation, so samples are recorded in-process — but no code anywhere in the repo mounts a `promhttp.Handler()` on any router (`server/server.go`'s single `mux.Router` only carries the routes registered in `api/api.go`). There is currently no way to scrape this histogram over HTTP without adding your own `promhttp` handler.
- **`/health` (the base route) has no history or trend data.** Every field (`memory_usage`, `cpu_stats`, `database_stats`, `uptime`, `goroutines`, `query_metrics`, and so on, per `internal/health/health.go`'s `HealthMetrics` struct) is a point-in-time snapshot computed on each call. There's no built-in time series, so external polling and storage — Prometheus scraping a custom exporter, or a cron job hitting `/health` and logging the JSON — is required for trend analysis.
- **`slow_queries`** in the `/health` response's `query_metrics` are query durations over 100 ms, hardcoded in `HealthCollector.RecordQuery` — not configurable via `monitoring.slow_query_threshold` (that key is unused, per above).
...
**What it is:** RocketVault can generate and operate RSA/ECDSA keys through any PKCS#11-compliant hardware security module (or a software token such as SoftHSM2), so private key material never enters Go process memory — it stays on the token, and only handles (labels) are stored in the database.

**Use it when:**

- Compliance requires private keys to be non-extractable and hardware-backed (for example, FIPS-adjacent deployments).
...
- For Postgres (Docker or Fly): `POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_DB`, `RV_MASTER_KEY`, `RV_JWT_SECRET`, and `RV_BOOTSTRAP_TOKEN` — all required, since `docker-entrypoint.sh` refuses to start the container if any is unset — plus `RV_ISSUER` and `RV_CORS_ORIGINS`.
