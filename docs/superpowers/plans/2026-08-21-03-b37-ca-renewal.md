...
Replace the generation block that follows the `certificateIsCA` call (the
...
**Leave the `isCA, err := certificateIsCA(original.Certificate)` block above it
in place**; the else branch reads that variable.
