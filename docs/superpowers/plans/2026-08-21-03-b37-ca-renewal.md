...
**Depends on:** `docs/superpowers/plans/2026-08-21-08-b43-ca-keyusage.md`
(**B43 and B44**), which **must land first**. It owns two things this plan
builds on:

- the CA `KeyUsageCertSign` fix that used to be Task 1 here (B43) — the
  chain-verification assertions in Tasks 6 and 8 cannot pass until it lands;
- the removal of the blanket `IsCA: true` on both self-signed call sites (B44).
  After plan 08, `RenewCertificate` reads the CA status out of the certificate
  it is replacing via `inspectCertificateCA`, a three-state helper
  (`caStatusLeaf` / `caStatusCA` / `caStatusUnsignableCA`) rather than a plain
  boolean — plan 08's own final review found that a naive preserve-the-flag
  boolean would have promoted every certificate issued before B43/B44 into a
  working CA on its first renewal, since those certificates assert `CA:TRUE`
  without `keyCertSign`. `caStatusUnsignableCA` catches that shape and demotes
  it to a leaf, audit-logged as `ca_demoted`, instead of preserving it. The
  fixture in `cert_soft_delete_test.go` already carries a real certificate
  body. **Task 7 below must preserve that three-state behavior, not overwrite
  it** — see the Task 1 stub and Decision 2.

Task 1 below is a stub kept in place so the task numbering — and every
cross-reference to it — stays valid.
...
2. **Must renewing a self-signed CA certificate keep `IsCA: true`?**
   **Superseded by plan 08 (B44) — read this whole item before Task 7.** The
   original answer was "yes, always", because `CreateSelfSignedCertificate` set
   `IsCA: true` unconditionally for *every* self-signed certificate and the
   renewal branch had to match create exactly. Plan 08 removed that blanket
   flag: a self-signed certificate is a CA only when the caller asks
   (`--is-ca` / `is_ca`), and `RenewCertificate` now reads the status out of
   the certificate it is replacing with
   `inspectCertificateCA(original.Certificate)`. That returns three states, not
   two — a genuine CA (`caStatusCA`), an ordinary leaf (`caStatusLeaf`), and a
   pre-fix certificate that asserts `CA:TRUE` without `keyCertSign`
   (`caStatusUnsignableCA`), which plan 08 added after its final review found
   that preserving that shape verbatim would silently arm the entire
   pre-B43/B44 fleet as working CAs on renewal. So the answer is now **preserve
   for a genuine CA or leaf, demote for the pre-fix shape**: a real CA renews
   as a CA (the original concern — stripping the bit off every CA on its first
   renewal — still holds), a leaf renews as a leaf (forcing `true` would
   re-create B44 one renewal at a time), and the pre-fix shape renews as a
   leaf with the demotion audit-logged as `ca_demoted`. Task 7's self-signed
   branch must pass that
   preserved value, not a literal.
...
**Plan 08 now covers B43 *and* B44, not B43 alone.** B44 is the blanket
`IsCA: true` on both self-signed call sites. The two had to ship together: B43's
fix adds `CertSign` conditioned on `IsCA`, so landing it while every self-signed
certificate was marked `CA:TRUE` would have given all of them working signing
authority. Plan 08 lands B44's default and its `--is-ca` opt-in first, then
B43's key usage.
...
#### What plan 08 changed under this plan's feet

Three places in this plan describe a tree that plan 08 has already moved. Read
these before implementing Task 7, and do not "restore" what looks like a
missing `IsCA: true` — it was removed on purpose.

1. **Decision 2 above is superseded.** Renewal preserves a genuine CA's or
   leaf's status; it does not force `true`, and it demotes the pre-fix
   `CA:TRUE`-without-`keyCertSign` shape to a leaf. Plan 08 added
   `inspectCertificateCA(certPEM string) (caStatus, error)` beside
   `extractExpiresAt` and calls it in `RenewCertificate` before generation.
2. **Task 7 Step 1 is already done.**
   `TestRenewCertificate_Succeeds_WhenKeyIDSet` already has a real certificate
   body, because plan 08's renewal change reads that body. Plan 08 issued it
   with `IsCA: false`, deliberately: a fixture certificate is an ordinary
   certificate. Verify rather than re-add; adding the block twice will not
   compile (`existingCertPEM` redeclared).
3. **Task 7 Step 5's self-signed branch must keep the preserved value.** The
   step's code block below already reflects this — it passes `IsCA: isCA`,
   where `isCA` is the variable plan 08 computes above the branch. Writing
   `IsCA: true` there instead would reintroduce B44 one renewal at a time, and
   with B43 now fixed each such renewal would produce a *working* CA.
   `TestRenewCertificate_PreservesNonCA` in
   `internal/services/certificates/ca_opt_in_test.go` fails loudly if it
   happens.

Also note that plan 08 created `internal/services/certificates/ca_opt_in_test.go`
in this package, whose PEM helper is named `parseIssuedCert` specifically so it
does not collide with the `parseCertPEM` that Task 6 Step 2 declares in
`chain_assert_test.go`. Keep both names.

- [ ] **Step 1: Confirm plan 08 has landed before starting Task 2**

```bash
go test ./internal/crypto/ -run 'TestCASignedChain_RSACA_Verifies' -v
go test ./internal/services/certificates/ -run 'TestRenewCertificate_PreservesNonCA' -v
```

Expected: both tests exist and pass. If either reports `no tests to run`, plan
08 has not landed yet — stop and implement it first.
...
- [ ] **Step 1: Confirm the renewal fixture already has a certificate body**

`TestRenewCertificate_Succeeds_WhenKeyIDSet` used to build `existingCert` with
no `Certificate` field, so the stored body was `""` — which no real row is, and
which the new self-signed branch cannot inspect. **Plan 08 already fixed this**,
because its renewal change reads the CA flag out of the same body. Verify
rather than re-apply:

```bash
grep -n "existingCertPEM" internal/services/certificates/cert_soft_delete_test.go
```

Expected: the fixture assigns `existingCertPEM` from
`crypto.CreateSelfSignedCertificatePEM(..., IsCA: false)` and sets
`Certificate: existingCertPEM` on `existingCert`. `IsCA: false` is deliberate —
a fixture certificate is an ordinary certificate, and renewal now preserves
whatever the stored body says.

If the grep finds nothing, plan 08 has not landed. Stop and land it first
rather than adding the block here; the two plans would otherwise both declare
`existingCertPEM`.
...
Replace the generation block that follows the `inspectCertificateCA` call (the
`crypto.CreateSelfSignedCertificatePEM(...)` call and its error handling —
locate it by name, plan 08 shifted the line numbers) with the branch below.
**Leave the `status, err := inspectCertificateCA(original.Certificate)` block
above it — including the `isCA := status == caStatusCA` and
`demoted := status == caStatusUnsignableCA` lines that derive from it — in
place**; the else branch reads `isCA`, and the audit logging after the store
reads `demoted`. Do not replace either derived value with a hardcoded
`IsCA: true` (or `false`) — see Decision 2 above and § B44 in
`.claude/known-bugs.md` for why that reintroduces the bug.
...
RenewCertificate called CreateSelfSignedCertificatePEM for every certificate,
so renewing a certificate issued against a CA returned a self-signed one and
every relying party that validated the chain rejected it -- with nothing in
the output saying the issuer had changed.
...
and a self-signed renewal still carries over the CA flag of the certificate it
replaces rather than forcing one."
```
