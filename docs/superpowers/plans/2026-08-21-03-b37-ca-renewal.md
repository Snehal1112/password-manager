...
- the removal of the blanket `IsCA: true` on both self-signed call sites (B44).
  After plan 08, `RenewCertificate` reads the CA flag out of the certificate it
  is replacing via `certificateIsCA`, and the fixture in
  `cert_soft_delete_test.go` already carries a real certificate body. **Task 7
  below must preserve that behavior, not overwrite it** — see the Task 1 stub
  and Decision 2.
...
2. **Must renewing a self-signed CA certificate keep `IsCA: true`?**
   **Superseded by plan 08 (B44) — read this whole item before Task 7.** The
   ...
   (`--is-ca` / `is_ca`), and `RenewCertificate` now reads the flag out of the
   certificate it is replacing with `certificateIsCA(original.Certificate)`.
   So the answer is now **preserve, never force**: a CA renews as a CA (the
   original concern — stripping the bit off every CA on its first renewal —
   still holds), and a leaf renews as a leaf (forcing `true` would re-create
   B44 one renewal at a time). Task 7's self-signed branch must pass that
...
1. **Decision 2 above is superseded.** Renewal preserves the original's CA
   status; it does not force `true`. Plan 08 added
   `certificateIsCA(certPEM string) (bool, error)` beside `extractExpiresAt`
   and calls it in `RenewCertificate` before generation.
