...
- The spec's headline test says the chain must verify with `CheckSignatureFrom`
  ...
  a `ConstraintViolationError`. Task 1 fixes that.
...
### Task 1: Give CA certificates the CertSign key usage

**Files:**
- Modify: `internal/crypto/x509_helper.go` (`CreateX509Template`, l.48-68; the `KeyUsage` assignment is l.61)
- Modify: `internal/crypto/crypto_operations_test.go` — no; create a new test file instead
- Create: `internal/crypto/x509_ca_keyusage_test.go`

**Interfaces:**
- Consumes: `crypto.CertificateTemplate{CommonName, ValidityDays, IsCA}` (unchanged).
- Produces: `CreateX509Template` now returns a template whose `KeyUsage`
  includes `x509.KeyUsageCertSign | x509.KeyUsageCRLSign` when `params.IsCA`
  is true. No signature change.
...
// A CA certificate must carry KeyUsageCertSign. RFC 5280 requires it, and
// Go's own verifier rejects a chain whose issuer has a non-zero KeyUsage
// without it -- which made every chain this system issued unverifiable.
...
// A leaf certificate must not silently gain signing authority.
...
// The end-to-end consequence: a leaf signed by a CA this package generated
// must satisfy CheckSignatureFrom, which enforces the CertSign constraint.
func TestCASignedCertificate_SatisfiesCheckSignatureFrom(t *testing.T) {
...
```bash
go test ./internal/crypto/ -run 'TestCreateX509Template_CAGetsCertSign|TestCASignedCertificate_SatisfiesCheckSignatureFrom' -v
```

Expected: `TestCreateX509Template_CAGetsCertSign` fails on
`Should not be zero, but was 0` for the `CertSign` assertion, and
`TestCASignedCertificate_SatisfiesCheckSignatureFrom` fails with
`x509: invalid signature: parent certificate cannot sign this kind of certificate`.

- [ ] **Step 3: Implement**

In `internal/crypto/x509_helper.go`, replace the body of `CreateX509Template`
between the serial-number call and the `return`:

```go
	// A CA must be allowed to sign certificates and CRLs. Without CertSign,
	// crypto/x509's own verifier rejects every chain this CA issues with a
	// constraint violation, so a certificate signed here would never verify.
...
Expected: all three new tests pass and no existing `internal/crypto` test regresses.
...
CreateX509Template set KeyUsage to DigitalSignature|KeyEncipherment for
every certificate, CA or not. crypto/x509 rejects a chain whose issuer has
a non-zero KeyUsage without KeyUsageCertSign, so both CheckSignatureFrom
and a cert-pool Verify failed with a constraint violation for every chain
this package ever issued -- the signature itself was fine, the policy bits
were not. CA templates now add CertSign and CRLSign; leaf templates are
unchanged.

Certificates issued before this change keep the old usage bits. Renewing a
CA re-issues it with the correct ones."
```
...
**Files:**
- Modify: `internal/crypto/x509_helper.go` (`CreateCASignedCertificatePEM`, l.131-193; the type switch is l.161-179)
- Modify: `internal/crypto/x509_ca_keyusage_test.go` (add one test)
