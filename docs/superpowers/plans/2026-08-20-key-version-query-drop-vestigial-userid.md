...
- **`CurrentVersion` drops its `LEFT JOIN keys k` entirely, along with the `AND k.user_id = ?` term.** (Corrected during final review, 2026-08-20: the join was never load-bearing. An ungrouped aggregate query always returns exactly one row regardless of join type or how many rows match, so `COALESCE(MAX(version), 1)` against `key_versions` alone already returns 1 for a never-rotated key with zero `key_versions` rows — no join from `keys` is needed.) All five methods select only from `key_versions` and drop their join entirely.
