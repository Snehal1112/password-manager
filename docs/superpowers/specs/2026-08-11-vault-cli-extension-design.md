...
- **No admin short-circuit in `RequireDataAction`**, unlike the sibling `CanManageVault`/`CanPurgeVault` functions (`docs/superpowers/plans/2026-08-11-02-shared-vault-authz-functions.md`) — those two do short-circuit for `admin`, matching `PolicyMiddleware`'s vault-*management* behavior, but data-plane access has no such bypass today, confirmed by reading `PolicyMiddleware`'s vault-data-plane branch directly.
